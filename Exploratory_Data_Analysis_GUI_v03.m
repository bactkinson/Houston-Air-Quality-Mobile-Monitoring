function varargout = Exploratory_Data_Analysis_GUI_v03(varargin)
% EXPLORATORY_DATA_ANALYSIS_GUI_v03 MATLAB code for Exploratory_Data_Analysis_GUI_v03.fig
%      EXPLORATORY_DATA_ANALYSIS_GUI_v03, by itself, creates a new EXPLORATORY_DATA_ANALYSIS_GUI_v03 or raises the existing
%      singleton*.
%
%      H = EXPLORATORY_DATA_ANALYSIS_GUI_v03 returns the handle to a new EXPLORATORY_DATA_ANALYSIS_GUI_v03 or the handle to
%      the existing singleton*.
%
%      EXPLORATORY_DATA_ANALYSIS_GUI_v03('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in EXPLORATORY_DATA_ANALYSIS_GUI_v03.M with the given input arguments.
%
%      EXPLORATORY_DATA_ANALYSIS_GUI_v03('Property','Value',...) creates a new EXPLORATORY_DATA_ANALYSIS_GUI_v03 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Exploratory_Data_Analysis_GUI_v03_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Exploratory_Data_Analysis_GUI_v03_OpeningFcn via varargin.
%
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @Exploratory_Data_Analysis_GUI_v03_OpeningFcn, ...
                   'gui_OutputFcn',  @Exploratory_Data_Analysis_GUI_v03_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Exploratory_Data_Analysis_GUI_v03 is made visible.
function Exploratory_Data_Analysis_GUI_v03_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Exploratory_Data_Analysis_GUI_v03 (see VARARGIN)
...
% Choose default command line output for Exploratory_Data_Analysis_GUI_v03
...
% UIWAIT makes Exploratory_Data_Analysis_GUI_v03 wait for user response (see UIRESUME)
...
tdates =  datestr(unique(floor(tdatatemp(idx_data)))); %% CHANGE WAS HERE
...
function varargout = Exploratory_Data_Analysis_GUI_v03_OutputFcn(hObject, eventdata, handles) 
...
if ~isfield(UserData,'units')
    error('Units are a required field!')
    % units = ''
else
    units=UserData.units;
end
...
if isfield(UserData,'DrivePassIndex')
    index_pass = UserData.DrivePassIndex; 
    index_pass = index_pass(Index_Final);
...
min_days =  handles.Min_Days_Text.Value;
n_days = accumarray(index,floor(tdata),[],@(x)length(unique(x)),NaN);
idx_days = n_days >= min_days;
